...
class InteractiveSelector:
    ...
    def _fallback_selection(self, prompt: str, allow_cancel: bool) -> Any | None:
        ...
        while True:
            try:
                if allow_cancel:
                    user_input = input(f"Select (1-{len(self.items)}) or 'c' to cancel: ").strip().lower()
                    if user_input in ['c', 'cancel', 'q', 'quit']:
                        return None
                else:
...
class BackupSelector(InteractiveSelector):
    ...
    def _format_backup(self, backup: BackupInfo) -> str:
        ...
        files_info = f"{len(backup.files_backed_up)} editor(s)" if backup.files_backed_up else "0 editors"
...
def confirm_action(message: str, default: bool = False, force: bool = False) -> bool:
    ...
    try:
        response = input(f"{Fore.YELLOW}{message} {choices}: {Style.RESET_ALL}").strip().lower()
...
def select_backup(
    ...
    prompt: str = "Select backup to restore",
...
def get_backup_description() -> str | None:
    ...
    """
    try:
        description = input(f"{Fore.CYAN}Enter backup description (optional): {Style.RESET_ALL}").strip()
