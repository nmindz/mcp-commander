...
def get_command_examples() -> dict[str, list[str]]:
    ...
    return {
        "add": [
            ...
            'mcp add server --all github \'{"transport": {"type": "http", "url": "https://api.githubcopilot.com/mcp/", "headers": {"Authorization": "Bearer $GITHUB_MCP_PAT"}}}\' --from-env=GITHUB_MCP_PAT',
            'mcp add server github-simple \'{"transport": {"type": "http", "url": "https://api.githubcopilot.com/mcp/"}}\' --header "Authorization:Bearer $GITHUB_MCP_PAT"',
